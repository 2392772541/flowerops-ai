...
export const demoState: AppState = {
  ...
  proposals: [
    { id: 'AP-001', type: 'CREATE_PURCHASE_ORDER', title: '补充 260 枝红玫瑰，覆盖周末订单', reason: '未来三天已确认花束订单将消耗 342 枝红玫瑰；临期批次有效库存折损后，预计周六出现 118 枝缺口。', evidence: [{ label: '未来需求', value: '342 枝', detail: '18束已确认订单 × 19枝配方' }, { label: '有效库存', value: '356 枝', detail: '可售库存按新鲜度系数折算' }, { label: '安全库存', value: '180 枝', detail: '当前演示配置，可由经营者调整' }], payload: { supplierId: 'S-YN01', productId: 'P-ROSE-R', quantity: 260, unitCost: 2.9 }, expectedImpact: '将周末预测缺货率从 23% 降至 4%，预计占用采购预算 ¥754。', risk: 'MEDIUM', status: 'AWAITING_APPROVAL', idempotencyKey: 'purchase-rose-20260904-v1', generatedAt: '2026-09-04T09:30:00+08:00' },
    { id: 'AP-002', type: 'CREATE_WASTE_PROPOSAL', title: '复核向日葵临期批次并制定促销去化', reason: '批次 GX-S-0901 明日到可售期限，剩余可售 48 枝；按近四周同星期销量，售罄概率不足 45%。', evidence: [{ label: '剩余可售', value: '48 枝', detail: '105在库 - 49预留 - 8过期' }, { label: '剩余时间', value: '1 天', detail: '可售期限为 2026-09-05' }, { label: '风险成本', value: '¥106', detail: '库存成本 × 预计未售概率' }], payload: { lotId: 'LOT-S01', suggestedDiscount: 0.15 }, expectedImpact: '优先促销可回收约 ¥260 销售额，并减少潜在损耗。', risk: 'LOW', status: 'AWAITING_APPROVAL', idempotencyKey: 'waste-sun-20260904-v1', generatedAt: '2026-09-04T09:35:00+08:00' },
