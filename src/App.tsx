...
function Decisions({ state, setState }: { state: AppState; setState: (s: AppState) => void }) {
  ...
  return <div className="decision-layout">
    ...
    {selected && <section className="panel proposal-detail"><div className="detail-head"><div><Badge tone={selected.risk.toLowerCase()}>{riskLabel(selected.risk)}</Badge><h2>{selected.title}</h2><p>{selected.reason}</p></div><Badge tone={statusClass(selected.status)}>{statusLabels[selected.status]}</Badge></div>
      ...
      {selected.executionResult && <div className="result-banner"><Check size={18}/><div><b>执行结果</b><p>{selected.executionResult}</p></div></div>}
      <div className="approval-bar"><div><ShieldCheck size={18}/><span>所有高影响动作均需人工批准，并写入审计日志。</span></div><div>{selected.status === 'AWAITING_APPROVAL' && <><button className="ghost danger-text" onClick={()=>setState(rejectProposal(state, selected.id))}><X size={16}/>拒绝</button><button className="primary" onClick={()=>setState(approveProposal(state, selected.id))}><Check size={16}/>批准提案</button></>}{selected.status === 'APPROVED' && <button className="primary" onClick={()=>setState(executeProposal(state, selected.id))}><PackageCheck size={17}/>安全执行</button>}</div></div>
    </section>}
  </div>
}
